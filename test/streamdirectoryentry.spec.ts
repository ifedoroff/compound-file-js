...
import {initializedWith} from "../src/utils";
...
describe('Stream Directory Entry test', () => {
    it('read mini stream data', () => {
        ...
        const bytes = initializedWith(65, 0);
        bytes[64] = 1;
    ...
    it('read regular stream data', () => {
        const compoundFile = new CompoundFile();
        const storage = compoundFile.getRootStorage().addStorage("test");
        let bytes = initializedWith(4096, 0);
        bytes[4093] = 2;
        bytes[4094] = 3;
        bytes[4095] = 4;
    ...
    it('writeAt method of mini stream', () => {
        ...
        expect(() => finalMiniStream.writeAt(0, initializedWith(9, 0))).throw();
        expect(() => finalMiniStream.writeAt(9, initializedWith(1, 0))).throw();
        expect(() => finalMiniStream.writeAt(-1, initializedWith(1, 0))).throw();
        finalMiniStream.writeAt(2, [15]);
        expect(finalMiniStream.read(2, 3)).to.deep.eq([15]);

        const miniStream1 = storage.addStream("mini1", initializedWith(65, 0));
    ...
    it('writeAt method of regular stream', () => {
        ...
        const storage = compoundFile.getRootStorage().addStorage("test");
        const finalMiniStream = storage.addStream("mini", [0, 1, 2, 3, 4, 5, 6, 7]);
        expect(() => finalMiniStream.writeAt(0, initializedWith(9, 0)));
        expect(() => finalMiniStream.writeAt(9, initializedWith(1, 0)));
        expect(() => finalMiniStream.writeAt(-1, initializedWith(1, 0)));
        const bytes = initializedWith(4096, 0);
        finalMiniStream.setStreamData(bytes);
        expect(() => finalMiniStream.writeAt(0, initializedWith(4097, 0)));
        expect(() => finalMiniStream.writeAt(4096, initializedWith(1, 0)));
        expect(() => finalMiniStream.writeAt(-1, initializedWith(1, 0)));
        finalMiniStream.writeAt(2, [15]);
        expect(finalMiniStream.read(2, 3)).to.deep.eq([15]);

        const miniStream1 = storage.addStream("mini1", initializedWith(4097, 0));
    ...
    it('append to mini stream', () => {
        ...
        mini = compoundFile.getRootStorage().addStream("mini1", initializedWith(64, 1));
        mini.append([2,3,4]);
        expect(mini.getStreamSize()).eq(67);
        expect(mini.read(64, 67)).to.deep.eq([2,3,4]);
        mini = compoundFile.getRootStorage().addStream("mini2", initializedWith(63, 1));
        mini.append([0]);
    ...
    it('apend to regular stream', () => {
        const compoundFile = new CompoundFile();
        let mini = compoundFile.getRootStorage().addStream("mini", initializedWith(4100, 0));
        mini.append([3,4,5]);
        expect(mini.getStreamSize()).eq(4103);
        expect(mini.read(4100, 4103)).to.deep.eq([3, 4, 5]);
        mini = compoundFile.getRootStorage().addStream("mini1", initializedWith(4096, 1));
        mini.append([2,3,4]);
    ...
    it('append data so that mini stream converts to regular stream (this happens once stream passes threshold in 4096 bytes)', () => {
        const compoundFile = new CompoundFile();
        const miniStream = compoundFile.getRootStorage().addStream("mini", initializedWith(4095, 0));
        miniStream.append([1]);
